...
class MyTokenizer(object):
    def __init__(self, pretrained_model_path=None, mask_entity=False):
        self.pretrained_model_path = pretrained_model_path or 'bert-base-english'
    ...
    def tokenize(self, item):
        ...
        sentence = item['text']
        # print("Original sequence: ", sentence)
        # tokenized_sequence = self.bert_tokenizer.tokenize(sentence)
        # print("Tokenized sequence: ", tokenized_sequence)
        # encodings = self.bert_tokenizer(sentence)
        # encoded_sequence = encodings['input_ids']
        # print("Encoded sequence: ", encoded_sequence)
        # decoded_encodings = self.bert_tokenizer.decode(encoded_sequence)
        ...
        sent0 = self.bert_tokenizer.tokenize(" ".join(text[:pos_min[0]]))
        ent0 = self.bert_tokenizer.tokenize(" ".join(text[pos_min[0]:pos_min[1]]))
        sent1 = self.bert_tokenizer.tokenize(" ".join(text[pos_min[1]:pos_max[0]]))
        ent1 = self.bert_tokenizer.tokenize(" ".join(text[pos_max[0]:pos_max[1]]))
        sent2 = self.bert_tokenizer.tokenize(" ".join(text[pos_max[1]:]))
